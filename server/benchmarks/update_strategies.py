...
def get_update_strategies(n_dims: int, n_chunks: int, max_age: int) -> list[tuple[str, Callable[[], OutdatedItemSelectionStrategy]]]:
  return list(
    map(
      lambda label: (label, lambda: get_update_strategy(label, n_dims, n_chunks, max_age)),
      UPDATE_STRATEGY_LABELS
    )
  )


def get_update_strategy(label: str, n_dims: int, n_chunks: int, max_age: int) -> OutdatedItemSelectionStrategy:
  if label == "no update":
    ...
    return OutdatedBinUpdate(n_dims=n_dims, storage=None)
