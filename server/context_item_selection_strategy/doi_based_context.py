    ...
    def get_context_ids(self, n: int, current_chunk: int) -> np.ndarray:
        ...
        for i in range(self.n_bins):
            ...
            picks = sample_without_replacement(
                n_population=n_population, n_samples=n_samples
            )
            outdated_ids_bin = ids_in_bin.iloc[picks][ID].to_numpy()
            outdated_ids = np.append(outdated_ids, outdated_ids_bin)

        # some bins may be smaller than the number of samples we want to draw from them, so there
        # can be "open spots" that we can fill randomly
