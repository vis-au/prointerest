...
class ProgressiveBinSampler:
    ...
    def get_current_sample(self, return_labels=False):
        ...
        X_sample = np.concatenate(
            tuple([self.X_reservoir[x] for x in self.X_reservoir])
        )
        ...
        y_sample = np.concatenate(
            tuple([self.y_reservoir[x] for x in self.y_reservoir])
        )
    ...
    def _reservoir_sample_by_bin(
        self, X: np.ndarray, y: np.ndarray, bins: np.ndarray, processed: int
    ):
    ...
    def add_chunk(
        self, X: np.ndarray, y: np.ndarray, processed: int, compute_edges=False
    ):
