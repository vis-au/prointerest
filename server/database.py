...
TIMESTAMP = (
    "timestamp_"  # column in a table storing a timestamp for when a datum was updated
)
...
def create_tables(
    ...
    cursor.execute(
        f"CREATE TABLE {PROCESSED_DB} "
        f"({ID} VARCHAR UNIQUE PRIMARY KEY, {CHUNK} INTEGER)"
    )
    cursor.execute(
        f"CREATE TABLE {DOI_DB} " f"({ID} VARCHAR UNIQUE PRIMARY KEY,{DOI} VARCHAR)"
    )
    cursor.execute(
        f"CREATE TABLE {LAST_UPDATE_DB} "
...
def mark_ids_processed(ids: List):
    ...
    query = (
        f"INSERT INTO {LAST_UPDATE_DB} ({ID}, {TIMESTAMP}) VALUES {timestamped_values}"
    )
    cursor.execute(query)
...
def get_from_db(
    db_name: str, query_filters: List, dimensions: List, distinct=False, as_df=False
):
    where_clause = ""
...
def save_dois(ids: List, dois: List):
    ...
    for i, id in enumerate(ids):
        ...
        if i < len(ids) - 1:
            values += ","
    query = f"BEGIN TRANSACTION;INSERT INTO {DOI_DB} ({ID}, {DOI}) VALUES {values};COMMIT;"
    cursor.execute(query)
