...
<DoiConfig
  ...
  <p class="explanation">Info:
    { #if $selectedOutlierMeasure === "tukey" }
      The "tukey" measure captures outlierness by determining, if the item falls outside the Tukey ranges across <BigNumber>75%</BigNumber> of the data.
    { :else if $selectedOutlierMeasure === "scagnostic" }
      The "scagnostics" measure captures outlierness by determining if the item returns an outlierness scagnostic value of greater than <BigNumber>.95</BigNumber>.
    { :else if $selectedOutlierMeasure === "clustering" }
      The "clustering" measure captures outlierness by determining, if the item is assigned its own cluster when applying DBSCAN to the data.
    { /if }
  </p>
</DoiConfig>
